...
(function (Blip) {
  ...
  class FreezeMode {
    ...
    // ---------------- 액션 ----------------
    //  좌+우 동시클릭 → 영역 캡처. 우측 아톰 패널은 자동으로 열지 않고,
    //  "왜 캡처했는지" 메모 팝업만 띄운다.
    async _captureRegion() {
      ...
      const atom = new Blip.Atom({ image: img });
      this.atoms.addCapture(atom, false);              // open=false → 우측 아톰 안 뜸
      if (this.garden) this.garden.addCapture(atom);   // garden 캔버스에는 표시
      if (this.editor && this.editor.captureNote) {
        this.editor.captureNote(atom, {
          title: '📝 캡처 메모',
          placeholder: '이걸 왜 캡처했는지… (3초간 입력이 없으면 메모 없이 닫힘 · Enter 저장)',
        });
      }
    }

    //  1클릭 → 클릭한 영역에 대해 "왜 클릭했는지" 메모 팝업
    _clickNote() {
      this._pick(this.lastX, this.lastY);
      if (!this.selected) return;
      this._flashBox();
      const mark = { note: '' };                       // 메모만 담는 가벼운 객체
      if (this.editor && this.editor.captureNote) {
        this.editor.captureNote(mark, {
          title: '📝 클릭 메모',
          placeholder: '이걸 왜 클릭했는지… (3초간 입력이 없으면 메모 없이 닫힘 · Enter 저장)',
        });
      }
    }
    _deleteRegion() {
    ...
    // ---------------- 이벤트 ----------------
    //  단순화: FREEZE 에서는 (1) 좌 1클릭  (2) 좌+우 동시클릭  두 가지만 동작.
    //  - 1클릭        → "왜 클릭했는지" 메모 팝업
    //  - 좌+우 동시   → 영역 캡처(+메모 팝업, 우측 아톰 안 뜸)
    //  - Esc          → freeze 종료
    _bind() {
      this.overlay.addEventListener('mousedown', (e) => {
        ...
        if (e.button === 2) { this.rightDown = true; return; }  // 조합 감지용으로만 추적
      ...
      window.addEventListener('mousemove', (e) => {
        ...
        this._pick(e.clientX, e.clientY);     // 호버 하이라이트
      });
      ...
      window.addEventListener('mouseup', (e) => {
        ...
        if (e.button !== 0) return;
        if (!this.leftDown && !this.captureCombo) return;   // 패널 등에서 시작한 클릭 무시
        this.leftDown = false;
        if (this.captureCombo) { this.captureCombo = false; return; }
        if (this.moved) { this.moved = false; return; }     // 드래그였으면 무시(액션 없음)

        this._clickNote();                                  // 좌 1클릭 → 메모 팝업
      });
